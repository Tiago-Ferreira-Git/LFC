...
>>>>>>> 7ed45c8baf499c9855c492ee0bd319b4b80e9d16
    ...
    for i = 1:size(network,2)
        ...
        for j = 1:size(neighbours,1)
            ...
            if(bus_ss(neighbours(j,1),2) ~= size(index_ss(neighbours(j,1)):index_ss(neighbours(j,1)+1)-1,2))
                disp('Welp')
            end
    ...
    A_global(index_ss(end-1),cumsum(bus_ss(:,2))-1) = A_global(index_ss(end-1),size(A_global,1));
    A_global(:,end) = [];
    A_global(end,:) = [];
