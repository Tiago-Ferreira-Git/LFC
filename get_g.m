function [g,bus,line] = get_g(data_file)
    ...
    if exist('tg_con','var')
        ...
        mask_machines = bitor(bus(:,10)==2,bus(:,10)==1);
        n_machines = size(bus(mask_machines,1),1);
    
        if n_machines < size(g.tg.tg_con,1) 
            g.tg.tg_con = g.tg.tg_con(1:n_machines,:);
            

        else
            g.tg.tg_con = repmat(g.tg.tg_con ,floor(n_machines/size(g.tg.tg_con,1)),1);
            machines_left = mod(n_machines,size(g.tg.tg_con,1));
            g.tg.tg_con(end:end+machines_left,:) = g.tg.tg_con(end-machines_left:end,:);
        end
