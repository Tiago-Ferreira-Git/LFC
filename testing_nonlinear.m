...
for iteration = 1:3
    ...
    set(gca,'FontSize',20);
    set(gca,'TickLabelInterpreter','latex') % Latex style axis
    stairs(t_second,y_d(1:4:end,1:10:end)','LineWidth',1.5);
    ylim([0.9996 1.0006])
    xlabel('$t \;[\mathrm{s}]$','Interpreter','latex');
    ylabel('$\omega$ (pu)','interpreter','latex');
    xlabel('$t \;[\mathrm{s}]$','Interpreter','latex');
    ...
    set(gca,'FontSize',20);
    set(gca,'TickLabelInterpreter','latex') % Latex style axis
    stairs(t_second,(u0(:,1:10:end) + delta_u(:,1:10:end))','LineWidth',1.5);
    xlabel('$t \;(\mathrm{h})$','Interpreter','latex');
    ylabel('$\mathbf{u}$ (pu)','interpreter','latex');
    xlabel('$t \;(\mathrm{h})$','Interpreter','latex');
    savefig('./fig/u_d.fig');
...
xlabel('$\mathcal{S}(\mathbf{E})$','Interpreter','latex');
ylabel('$\frac{J_\mathrm{d}}{J_\mathrm{c}}$','interpreter','latex');
...
mask = (t_second < (t_zoom+1)*3600) & (t_second > -100 + (t_zoom)*3600); % range of t near perturbation
stairs(t_second(mask),y_c(1:4:end,mask)','LineWidth',1.5) % plot on new axes
%stairs(t_second(1:end-1),y_c(1:4:end,1:end-1)','LineWidth',1.5);
...
box on % put box around new pair of axes
mask = (t_second < (t_zoom_2+5)*3600) & (t_second > -100 + (t_zoom_2)*3600); % range of t near perturbation
stairs(t_second(mask),y_c(1:4:end,mask)','LineWidth',1.5) % plot on new axes
%stairs(t_second(1:end-1),y_c(1:4:end,1:end-1)','LineWidth',1.5);
...
box on % put box around new pair of axes
mask = (t_second < (t_zoom+1)*3600) & (t_second > -100 + (t_zoom)*3600); % range of t near perturbation
stairs(t_second(mask),y_d(1:4:end,mask)','LineWidth',1.5) % plot on new axes
xticks((t_zoom)*3600:3600:(t_zoom+1)*3600)
xticklabels(sprintfc('%d', t_zoom:t_zoom+1))
axis padded
yline(1+4e-3)
...
set(gca,'FontSize',20);
set(gca,'TickLabelInterpreter','latex') % Latex style axis
stairs(t_L(1:10:end),(u0(:,1:10:end) + delta_u_decentralized(:,1:10:end))','LineWidth',1.5);
