...
for i = 1:size(network,2)
    ...
    %length(unique_areas) - length(unique_areas_) 
...
K_order = zeros(3*size(B,2),3*size(A,2));
trace_order = zeros(3*size(B,2),100000);
...
for j = 1:3
    ...
    K_order((j-1)*size(B,2)+1:j*size(B,2),(j-1)*size(A,2)+1:j*size(A,2)) = K;
    trace_order(j,:) = trace_records;
    [~,S,~] = dlqr(A,B,diag(q),R);
    ...
    %title(sprintf('t_{sh} = %.2f s',t_sh),'Interpreter','tex')
    ylabel('$\mathrm{tr}(\mathbf{P})$ ','interpreter','latex');
