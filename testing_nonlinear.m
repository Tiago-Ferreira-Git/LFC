...
savefig('./fig/poles14.fig');
set(gcf,'renderer','Painters');
saveas(gca,'./fig/poles14.eps','epsc');
...
x_method2 = zeros(size(A,1),size(t,2));
delta_u_method2  = zeros(size(B,2),size(t,2));
y_method2  = zeros(size(C,1),size(t,2));
x_method2(:,1) = zeros(size(A,1),1);
p_tie = zeros(2*n_areas,size(t,2));
...
for k = 1:length(t)-1
    ...
    x_method2(:,k+1) = A*x_method2(:,k) + B*delta_u_method2(:,k)+ W*w(:,k);
...
grid on
box on;
stairs(t,y_method2(1:3:end,:)','LineWidth',1.5);
% yline(1+freq_limit,'--');
% yline(1-freq_limit,'--');
ylabel('$\omega$ (pu)','interpreter','latex');
xlabel('$t \;[\mathrm{s}]$','Interpreter','latex');
