...
for j = 1:length(myFiles)
    ...
    opts.maxOLIt = 20;
    opts.W = 1500;
    opts.findWindowLength = false;
    %%
    R_ = 1e6;
    [K_cent,S] = dlqr(A,B,diag(q),R_*eye(size(B,2)));
    S = S*1.5;
    %[K, ~,trace_record]  = LQROneStepLTI(A,B,diag(q),R_*eye(size(B,2)),E,S);
    [Kinf,Pinf] = LQRFiniteHorizonLTI(A,B,diag(q),R_*eye(size(B,2)),E,S,K_cent,opts);
